...
function geraPopulacao() {
	...
	for (let index = 0; index < 8; index++) {
		populacao.push(geraNumeroRandomico(0, 1000));
	}
...
function calcularIndividuos() {
	...
	newArray = binaries.map((item) => {
		return {
			...
			fit: fitness(parseInt(item, 2), 6, 10),
		};
	});
...
function filtrarElementos() {
	...
	newArray = elements.filter(function (item) {
		return item.fit < 1000;
	});

	if (newArray.length === 0) {
		return "Nenhum individuo atende ao criterio(fit < 1000)";
	}

	console.log(`Individuos encontrados com fit < 1000:`);
